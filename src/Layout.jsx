          ...
          <div className="flex items-center justify-between h-20">
            ...
            <div className="hidden lg:flex lg:items-center lg:space-x-2">
              {navigationItems.map((item) => (
                <Link key={item.title} to={item.url} className={`px-6 py-3 rounded-2xl text-lg font-medium transition-all duration-200 ${location.pathname === item.url ? 'bg-blue-600 text-white shadow-lg' : 'text-gray-700 hover:bg-blue-100 hover:text-blue-700'}`}>
                  <item.icon className="w-5 h-5 inline mr-2" />{item.title}
                </Link>
              ))}
            </div>
            <div className="hidden lg:flex lg:items-center lg:space-x-4">
              <Link to={createPageUrl("Emergency")}>
                <Button className="bg-red-600 hover:bg-red-700 text-white text-lg px-6 py-3 rounded-2xl shadow-lg">
                  <AlertTriangle className="w-5 h-5 mr-2" />Emergency Help
                </Button>
              </Link>
            </div>
            <div className="lg:hidden flex items-center">
              <Button variant="outline" size="icon" onClick={() => setMobileMenuOpen(!mobileMenuOpen)} className="h-12 w-12 border-4 rounded-2xl">
                <span className="sr-only">Open main menu</span>{mobileMenuOpen ? <X className="block h-6 w-6" /> : <MenuIcon className="block h-6 w-6" />}
              </Button>
            </div>
          </div>
        </div>
        {mobileMenuOpen && (
          <div className="lg:hidden border-t-2 border-blue-100 bg-white">
            <div className="px-6 pt-4 pb-6 space-y-3">
              {navigationItems.map((item) => (
                <Link key={item.title} to={item.url} className={`block px-6 py-4 rounded-2xl text-lg font-medium transition-all duration-200 ${location.pathname === item.url ? 'bg-blue-600 text-white' : 'text-gray-700 hover:bg-blue-100 hover:text-blue-700'}`} onClick={() => setMobileMenuOpen(false)}>
                  <item.icon className="w-5 h-5 inline mr-3" />{item.title}
                </Link>
              ))}
              <Link to={createPageUrl("Emergency")} onClick={() => setMobileMenuOpen(false)} className="block">
                <Button className="w-full bg-red-600 hover:bg-red-700 text-white text-lg py-4 rounded-2xl"><AlertTriangle className="w-5 h-5 mr-2" />Emergency Help</Button>
              </Link>
            </div>
          </div>
        )}
